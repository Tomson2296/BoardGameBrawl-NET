...
namespace BoardGameBrawl.Persistence.Repositories.Entities.Player_Related.Schedule_Related
{
    public class PlayerScheduleRepository : GenericRepository<PlayerSchedule>, IPlayerScheduleRepository
    {
            ...
            else
            {
                ...
                foreach (var daily in dailyAvailabilities)
                {
                    ...
                    else
                    {
                        ...
                        if (existingAvailability.TimeSlots != null)
                        {
                            existingAvailability.TimeSlots.Clear();
                            foreach (var newSlot in daily.TimeSlots!)
                            {
                                existingAvailability.TimeSlots.Add(newSlot);
                            }
                        }
                    }
                }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        existingAvailability.TimeSlots.Clear();
                        foreach (var newSlot in availability.TimeSlots!)
                        {
                            existingAvailability.TimeSlots.Add(newSlot);
                        }
                    }
        ...
        public async Task<DailyAvailability?> GetDailyAvailabilityAsync(Guid playerId, DayOfWeek day, 
            CancellationToken cancellationToken = default)
        {
            cancellationToken.ThrowIfCancellationRequested();
            ArgumentNullException.ThrowIfNull(playerId);
            ...
            return await _context.DailyAvailabilities
                .Include(d => d.TimeSlots)
                .FirstOrDefaultAsync(d => d.DayOfWeek == day, cancellationToken);
        ...
        public async Task<TimeSlot?> GetTimeSlotAsync(int timeSlotId, CancellationToken cancellationToken = default)
        {
            cancellationToken.ThrowIfCancellationRequested();
            return await _context.Set<TimeSlot>().FindAsync(timeSlotId, cancellationToken);
        }
